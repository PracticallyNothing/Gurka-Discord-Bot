...
class AudioPlayerWrapper {
        ...
        songUrls.forEach((url) => {
            ...
            child.stdout.on('data', (data: string) => {
                ...
                this.musicChannel.send(
                    `+ Добавих ${numSongs} ${numSongs > 1 ? 'песни' : 'песен'} ${removedSongsString}.`);
    ...
    /**
      * @summary Grab the next song from the queue and plays it.
      */
    private playNextSong = async () => {
        ...
        log(`Got the following stream of type ${type}:\n  ${stream}`)
        const resource = createAudioResource(stream, { inputType: type });

        const normalizedTitle = this.currentSong.title.toLocaleLowerCase()
        const titleContains = (str: string) =>
            this.currentSong != null
            && normalizedTitle.indexOf(str.toLocaleLowerCase()) >= 0;
        ...
        if (titleContains('bladee')) {
            ...
            await this.musicChannel.send('ne gi puskaj tiq gluposti ve');
            this.skip();
            this.playNextSong();
        }
