...
addCmd(new ResponseCommand(CommandPrefix + 'help', 'Помагай, шефе!', [genHelp()]));
...
client.on('messageCreate', async (msg) => {
    ...
    if (msg.content == CommandPrefix + "kill")
        process.exit(-1)

    if (msg.content.startsWith(CommandPrefix + 'nick ')) {
        let self = await msg.guild!.members.fetch(client.user!.id);
        const name = msg.content.substring((CommandPrefix + 'nick').length)

        if (name.trim().length == 0)
            self.setNickname("");
        else
            self.setNickname(
                msg.content.substring((CommandPrefix + 'nick ').length));
